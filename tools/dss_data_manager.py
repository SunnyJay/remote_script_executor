...
class DssDataManager:
    ...
    def findMstpIdWhoseSeqIdIsInSection(self):
    ...
    def findSeqNumOfAllMstpid(self):
        dict = {}
        for cache_node in cache_address:
            master = sentinel_db1.master_for(cache_node)
            mstpid_list = master.keys('*')
    ...
    #查找是否存某个msgid及其位置
    def findWhereIsMsgId(self,msgId):
        isExists = False
        for cache_node in cache_address:
            master = sentinel.master_for(cache_node)
            ...
            if isExists == True:
                location = cache_node
                print 'msgId location:',cache_node
                break
        if isExists == False:
                print 'can not find msgId:',msgId
    ...
    #findWhereIsMsgId('4311744514-1458541121-00490')